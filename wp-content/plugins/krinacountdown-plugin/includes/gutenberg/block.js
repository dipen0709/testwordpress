( function( blocks, editor, i18n, element, components, _ ) {
	...
	blocks.registerBlockType( 'wp-countdown/countdown', {
		title: 'Wp Countdown',
		icon: "clock" ,
		category: 'common',
		...
		edit: function( props ) {
			...
			function create_open_hide_block(){
				...
				return el("div",{className:"wp_countdown_main_collapsible_element"+open_or_close_class},create_head(),create_content())
			}
			function create_head(){
				return el("div",{className:"head_block",onClick:function(value){open_close_element(value)}},
						  el("span",{className:"title_image"},
									 el("img",{src:wp_countdown_gutenberg['other_data']['content_icon']})
						  ),
						  el("span",{className:"title fdghdfg"},"Wp Countdown"
			...
			function create_content(){
				...
				wpda_countdown_fields.push(wpda_countdown_lb_simple_select("countdown_type",{"button":"Button","circle":"Circle","vertical_slide":"Vertical Slider"},"Countdown timer buttons type","Choose the Countdown buttons type."));
				...
				wpda_countdown_fields.push(wp_countdown_upgrate_pro_version());
				var table=el("tabel",{className:"wp_countdown_content_block"},wpda_countdown_fields)
				return el("div",{className:"wp_countdown_content_block"},table)
			}
			function open_close_element(colapsible_element){
				var target=colapsible_element.target;
				var head_element;
				if(target.parentNode.classList[0]=="wp_countdown_main_collapsible_element"){
				   head_element=target.parentNode;
				}
				if(target.parentNode.parentNode.classList[0]=="wp_countdown_main_collapsible_element"){
				   head_element=target.parentNode.parentNode;
				}
				if(target.parentNode.parentNode.parentNode.classList[0]=="wp_countdown_main_collapsible_element"){
				   head_element=target.parentNode.parentNode.parentNode;
				}
				if(target.parentNode.parentNode.parentNode.parentNode.classList[0]=="wp_countdown_main_collapsible_element"){
			...
			function wp_countdown_upgrate_pro_version(){
				return el("tr",{},
						 el("td",{colspan:"2",className:"wp_countdown_upgrate_pro_link_td"},
						   el("a",{href:"https://wp.com/wordpress-countdown-plugin",target:"_blank",style:{'text-align':'center','color': 'rgba(10, 154, 62, 1)','font-weight': 'bold','font-size':'18px', 'text-decoration': 'none'}},'Upgrade to Pro Version')
						   )
						 )
			}
		...
		save: function( props ) {	
			...
			return "[wp_countdown "+shortcode_atributes+"]"+props.attributes.expeiret_text+"[/wp_countdown]";
		}
