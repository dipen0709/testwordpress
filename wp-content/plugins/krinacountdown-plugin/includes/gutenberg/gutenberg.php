...
class wp_countdown_gutenberg{	
	...
	public function guthenberg_init(){
		...
		register_block_type( 'wp-countdown/countdown', array(
			'style' => 'wp_countdown_gutenberg_css',
			'editor_script' => 'wp_countdown_gutenberg_js',
		) );
		wp_add_inline_script(
			'wp_countdown_gutenberg_js',
			sprintf('var wp_countdown_gutenberg = { other_data: %s};',json_encode($this->other_dates(),JSON_PRETTY_PRINT)),
			'before'
		);
	}
