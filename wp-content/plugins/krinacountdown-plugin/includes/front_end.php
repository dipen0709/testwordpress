...
class wp_countdown_front_end{
	...
	function __construct($params){
		...
		//Add shortcode part
		add_shortcode( 'wp_countdown', array($this,'wp_wp_countdown_shortcode') );
	...
	public function wp_wp_countdown_shortcode( $atts,$content){
		self::$id_for_content++;
		$output_html='';
		...
		$output_html.='</div>';
		$output_html.='<script>'.$this->wp_countdown_javascript($curent_value).'</script><style>'.$this->wp_countdown_css($curent_value).'</style>';
		return	$output_html;		
	}
	
    /*############  Countdown JS function  ################*/
	
	public function wp_countdown_javascript($parametrs_for_countedown){
		$output_js='';
        ...
        jQuery(document).ready(function(){
			".((($day_left<=0 && $hourse_left<=0 && $minuts_left<=0 && $seconds_left<=0))?"jQuery('#main_countedown_".self::$id_for_content." .countdown').html('".htmlspecialchars($parametrs_for_countedown['content'])."')":"setInterval(function(){countdown_wp_timer('main_countedown_".self::$id_for_content."');},1000)")."
		});
		";
		return $output_js;
	}
	...
	public function wp_countdown_css($parametrs_for_countedown){
		$output_css='';
