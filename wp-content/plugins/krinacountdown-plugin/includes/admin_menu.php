...
class wp_countdown_admin_menu{
	...
	function __construct($param){
		...
		$this->include_files();
		// Insert button code
		add_action('media_buttons_context', array($this,'wp_countdown_button'));
		add_action('wp_ajax_wp_countdown_window_manager', array($this,'wp_countdown_window_insert_content'));
		
		add_action( 'wp_ajax_wp_countdown_page_save', array($this,'save_in_databese') );
		add_action( 'wp_ajax_wp_countdown_send_mail', array($this,'sending_mail') );
	...
	private function integrete_gutenberg(){
		$wp_countdown = new wp_countdown_gutenberg($this->plugin_url);
	}
	/*############################### Insert button function ########################################*/
	
	public function wp_countdown_button($context) {
	  
	  $img = $this->plugin_url. 'images/post_button.jpg';
	
	  $title = 'Add Countdown';	
	  $context .= '<a class="button thickbox" title="Create countdown and insert in posts/pages"    href="'.admin_url("admin-ajax.php").'?action=wp_countdown_window_manager&height=750&width=640">
	...
	public function wp_countdown_window_insert_content(){
		//wp_enqueue_script('jquery-ui-slider');
		?>
        <style>
        #miain_wp_countdown_window_manager > tbody > tr:nth-child(odd) {
		  background-color: rgba(176, 176, 176, 0.07);
		}
		#miain_wp_countdown_window_manager>tfoot>tr>td{
		...
		.pro_feature {
		  ...
		  color: rgba(10, 154, 62, 1);
			...
			<table id="miain_wp_countdown_window_manager" class="wp-list-table widefat fixed posts section_parametrs_table">  
                <tbody> 
                    <tr>
						<td>
                    ...
                    <tr>
						<td>
							Countdown timer buttons type <span title="Choose the Countdown buttons type." class="desription_class">?</span>
						</td>
						<td>
                           <select id="countdown_type" class="coming_set_hiddens">
                    ...
                    <tr class="tr_button tr_circle tr_vertical_slide">
                     ...
                     <tr  class="tr_button tr_circle tr_vertical_slide">
						...
						<td>
							<?php wp_countdown_setting::generete_fonts('countdown_font_famaly',"monospace") ?>
						</td>                
					</tr> 
                    <tr>
						<td>
						...
						<td>
							<?php wp_countdown_setting::generete_animation_select('countdown_animation_type','none'); ?>
						</td>                
					</tr>
                </tbody>
                <tfoot>
                	<tr>                      
                        <td colspan="2">
                        	...
                        	<span style="float:right"><a href="http://wp.com/wordpress-countdown-plugin" target="_blank" style="color: rgba(10, 154, 62, 1);; font-weight: bold; font-size: 18px; text-decoration: none;">Upgrade to Pro Version</a><br></span>
                        </td>                
                    </tr>
                </tfoot>
            </table>         
    ...
    <script type="text/javascript">
       ...
       jQuery('#TB_window').css('max-height',(jQuery('#miain_wp_countdown_window_manager').height()+66)+'px');
	   jQuery('#TB_ajaxContent').css('max-height',(jQuery('#miain_wp_countdown_window_manager').height()+16)+'px');
	   jQuery('#miain_wp_countdown_window_manager').ready(function(e) {
        ...
        function insert_countdown() {          
                ...
                tagtext = '[wp_countdown '+variables+']'+jQuery('#expeiret_text').val()+'[/wp_countdown]';
...
die;	
	...
	public function menu_requeried_scripts(){
		wp_enqueue_style('wp-countdown-admin-style');
	}	
	...
	?>
   <h2 class="headin_countdown">WordPress Countdown General Page <a target="blank" href="<?php echo wp_countdown_support_url; ?>" class="wp_support">Support</a></h2>
   <style>
   .headin_countdown{
		font-size: 23px;
		font-weight: 400;
	...
	public function featured_plugins(){
		$plugins_array=array(
			'countdown_extended'=>array(
						...
						'site_url'		=>	'http://wp.com/wordpress-countdown-extended-version',
		    ...
		    'gallery_album'=>array(
						...
						'site_url'		=>	'http://wp.com/wordpress-gallery-plugin',
			...
			'coming_soon'=>array(
						...
						'site_url'		=>	'http://wp.com/wordpress-coming-soon-plugin/',
			...
			'Contact forms'=>array(
						...
						'site_url'		=>	'http://wp.com/wordpress-contact-form-plugin/',
			...
			'Booking Calendar'=>array(
						...
						'site_url'		=>	'http://wp.com/wordpress-booking-calendar-plugin/',
			...
			'youtube'=>array(
						...
						'site_url'		=>	'http://wp.com/wordpress-youtube-embed-plugin',
            ...
            'facebook-comments'=>array(
						...
						'site_url'		=>	'http://wp.com/wordpress-facebook-comments-plugin/',
			...
			'lightbox'=>array(
						...
						'site_url'		=>	'http://wp.com/wordpress-lightbox-plugin',
			...
			'facebook'=>array(
						...
						'site_url'		=>	'http://wp.com/wordpress-facebook-like-box-plugin',
			...
			'poll'=>array(
						...
						'site_url'		=>	'http://wp.com/wordpress-polls-plugin',
        ...
        jQuery(window).resize(wp_countdown_feature_resize);
		jQuery(document).ready(function(e) {
            wp_countdown_feature_resize();
        });
		
		function wp_countdown_feature_resize(){
			var wp_countdown_width=jQuery('.featured_plugin_main').eq(0).parent().width();
			var count_of_elements=Math.max(parseInt(wp_countdown_width/450),1);
			var width_of_plugin=((wp_countdown_width-count_of_elements*24-2)/count_of_elements);
