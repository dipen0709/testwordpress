...
class wp_countdown extends WP_Widget {
	private static $id_for_content=0;
	// Constructor //	
	function __construct() {		
		$widget_ops = array( 'classname' => 'wp_countdown', 'description' => 'Countdown timer for widget ' ); // Widget code
		$control_ops = array( 'id_base' => 'wp_countdown' ); // Widget Controls
		parent::__construct( 'wp_countdown', 'Countdown', $widget_ops, $control_ops ); // Create the widget
	...
	function widget($args, $instance) {
		...
		// Widget output //
		echo $this->wp_generete_front_end($instance);
		// After widget part//
...
/*############################### Update settings Function #######################################*/	
	...
	function form($instance) {
        ...
        <br>
        <input type="text" id="<?php echo $this->get_field_id('end_time_date'); ?>" name="<?php echo $this->get_field_name('end_time_date'); ?>" value="<?php echo $instance['end_time_date'] ?>" class="wp-date-time-picker" /><small>dd-mm-yyyy hh:ii</small>
        </p>
        ...
        <p class="flb_field tr_button tr_circle tr_vertical_slide">
          ...
          <br>
          <?php wp_countdown_setting::generete_fonts('font_famely','monospace') ?>
        </p>
         <p class="flb_field">
          <label for="animation">Countdown animation type:<span class="pro_feature"> (pro)</span></label>
          <br>
          <?php wp_countdown_setting::generete_animation_select('animation','monospace') ?>
        </p>
        <br>
        <input type="hidden" id="flb-submit" name="flb-submit" value="1">
        <script>
        jQuery('.wp-date-time-picker').ready(function(){
				var nowTemp = new Date();
				var now = new Date(nowTemp.getFullYear(), nowTemp.getMonth(), nowTemp.getDate(), 0, 0, 0, 0);
				jQuery('.wp-date-time-picker').fdatepicker({
        ...
        <a href="http://wp.com/wordpress-countdown-plugin/" target="_blank" style="color: rgba(10, 154, 62, 1);; font-weight: bold; font-size: 18px; text-decoration: none;">Upgrade to Pro Version</a>
		<?php 
	}
	
	/*############################### Function for generating front-end ########################################*/
	
	private function wp_generete_front_end($parametrs){
		self::$id_for_content++;
		$output_html='';			
        ...
        jQuery(document).ready(function(){
			".((($day_left<=0 && $hourse_left<=0 && $minuts_left<=0 && $seconds_left<=0))?"jQuery('#main_countedown_widget_".self::$id_for_content." .countdown').html('".htmlspecialchars($parametrs['content'])."')":"setInterval(function(){countdown_wp_timer('main_countedown_widget_".self::$id_for_content."');},1000)")."
		});
		";
...
add_action('widgets_init', create_function('', 'return register_widget("wp_countdown");'));
